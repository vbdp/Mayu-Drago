...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    #endif
    float glowInTheDarkMultiplier0 = calculateGlowInTheDark((0.0 /*_GITDEMinLight*/), (1.0 /*_GITDEMaxLight*/), (0.0 /*_GITDEMinEmissionMultiplier*/), (1.0 /*_GITDEMaxEmissionMultiplier*/), (0.0 /*_EnableGITDEmission*/), (0.0 /*_GITDEWorldOrMesh*/));
    #if defined(PROP_EMISSIONMAP) || !defined(OPTIMIZER_ENABLED)
        
        if (!(0.0 /*_EmissionCenterOutEnabled*/))
        {
            emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiMesh.uv[(1.0 /*_EmissionMapUV*/)], float4(0,0,0,0)).rgb * lerp(1, baseColor, (1.0 /*_EmissionBaseColorAsMap*/)).rgb * float4(1,1,1,1).rgb;
        }
        else
        {
            emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(3,3,0,0).xy) + _Time.x * (5.0 /*_EmissionCenterOutSpeed*/)).rgb * lerp(1, baseColor, (1.0 /*_EmissionBaseColorAsMap*/)).rgb * float4(1,1,1,1).rgb;
        }
    #else
        emissionColor0 = lerp(1, baseColor, (1.0 /*_EmissionBaseColorAsMap*/)).rgb * float4(1,1,1,1).rgb;
    #endif
    ...
    if ((0.0 /*_ScrollingEmission*/))
    {
        ...
        {
            #if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
                emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, TRANSFORM_TEX(poiMesh.uv[(1.0 /*_EmissionMapUV*/)], _EmissionScrollingCurve) + (dot(pos, float4(0,-10,0,0).xyz) * (10.0 /*_EmissiveScroll_Interval*/)) + _Time.x * (10.0 /*_EmissiveScroll_Velocity*/)).r;
            #endif
        }
        else
        {
