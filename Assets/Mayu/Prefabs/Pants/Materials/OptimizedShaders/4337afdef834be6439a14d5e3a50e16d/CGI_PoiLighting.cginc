...
UnityLight CreateLight(float3 normal, fixed detailShadowMap)
{
    ...
    light.color = saturate(_LightColor0.rgb * lerp(1, poiLight.attenuation, (0.0 /*_AttenuationMultiplier*/)) * detailShadowMap);
...
float FadeShadows(float attenuation)
{
            ...
            attenuation = lerp(1, poiLight.attenuation, (0.0 /*_AttenuationMultiplier*/));
        #endif
...
void ApplySubtractiveLighting(inout UnityIndirect indirectLight)
{
        ...
        poiLight.attenuation = FadeShadows(lerp(1, poiLight.attenuation, (0.0 /*_AttenuationMultiplier*/)));
...
UnityIndirect CreateIndirectLight(float3 normal)
{
    ...
    indirectLight.specular = 0;
        ...
        envData.roughness = 1 - (0.0 /*_LightingStandardSmoothness*/);
        ...
        {
            occlusion = lerp(1, POI2D_SAMPLER_PAN(_LightingAOTex, _MainTex, poiMesh.uv[(0.0 /*_LightingAOTexUV*/)], float4(0,0,0,0)).r, (1.0 /*_AOStrength*/));
        }
        indirectLight.diffuse *= occlusion;
        indirectLight.diffuse = max(indirectLight.diffuse, (0.005 /*_LightingMinLightBrightness*/));
...
void calculateBasePassLightMaps()
{
        ...
        shadowStrength = 1;
        #ifndef OUTLINE
            shadowStrength = POI2D_SAMPLER_PAN(_LightingShadowMask, _MainTex, poiMesh.uv[(0.0 /*_LightingShadowMaskUV*/)], float4(0,0,0,0)) * (0.75 /*_ShadowStrength*/);
        #else
            shadowStrength = (0.0 /*_OutlineShadowStrength*/);
        #endif
        ...
        directLighting = lerp(directLighting, dot(directLighting, float3(0.299, 0.587, 0.114)), (0.1 /*_LightingMonochromatic*/));
        indirectLighting = lerp(indirectLighting, dot(indirectLighting, float3(0.299, 0.587, 0.114)), (0.1 /*_LightingMonochromatic*/));
        ...
        switch((0.0 /*_LightingRampType*/))
        {
            case 0: // Ramp Texture
            {
                ...
                if ((1.0 /*_LightingIgnoreAmbientColor*/))
                {
            ...
            case 1: // Math Gradient
            {
                ...
                if ((1.0 /*_LightingIgnoreAmbientColor*/))
                {
                    poiLight.finalLighting = lerp((directLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - (0.75 /*_ShadowStrength*/)));
                }
                else
                {
                    poiLight.finalLighting = lerp((indirectLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - (0.75 /*_ShadowStrength*/)));
                }
            }
            break;
            case 2:
        ...
        if ((1.0 /*_LightingMode*/) == 2) // Wrapped
        {
            ...
            {
                poiLight.rampedLightMap = lerp(float4(1,1,1,1) * lerp(poiLight.indirectLighting, 1, (1.0 /*_LightingIgnoreAmbientColor*/)), float3(1, 1, 1), saturate(1 - smoothstep((0.0 /*_LightingGradientStart*/) - .000001, (0.5 /*_LightingGradientEnd*/), 1 - poiLight.lightMap)));
                poiLight.rampedLightMap = lerp(float3(1, 1, 1), poiLight.rampedLightMap, shadowStrength.r);
            }
            poiLight.finalLighting = (poiLight.indirectLighting + poiLight.directLighting) * saturate(poiLight.rampedLightMap + 1 - (0.75 /*_ShadowStrength*/));
        }
        if (!(0.0 /*_LightingUncapped*/))
        {
...
float3 calculateNonImportantLighting(float attenuation, float attenuationDotNL, float3 albedo, float3 lightColor, half dotNL, half correctedDotNL)
{
    ...
    {
        return lerp(lightColor * attenuation, lightColor * (0.005 /*_LightingAdditivePassthrough*/) * attenuation, smoothstep((0.3 /*_LightingAdditiveGradientStart*/), (0.5 /*_LightingAdditiveGradientEnd*/), dotNL)) * detailShadow;
    }
...
void applyShadeMaps(inout float4 albedo)
{
    ...
    if ((0.0 /*_LightingRampType*/) == 2)
    {
        ...
        float firstSecondShadowMask = saturate(1 - ((poiLight.lightMap) - firstColorFeatherStep) / ((0.0 /*_ShadeColor_Step*/) - firstColorFeatherStep) * (shadowMask));
            ...
            float removeShadow = POI2D_SAMPLER_PAN(_LightingShadowMask, _MainTex, poiMesh.uv[(0.0 /*_LightingShadowMaskUV*/)], float4(0,0,0,0)).r;
        #else
...
float3 calculateFinalLighting(inout float3 albedo, float4 finalColor)
{
    float3 finalLighting = 1;
        ...
        else if ((1.0 /*_LightingAdditiveType*/) == 1) // Toon
        {
                ...
                finalLighting = lerp(poiLight.color * max(poiLight.additiveShadow, (0.005 /*_LightingAdditivePassthrough*/)), poiLight.color * (0.005 /*_LightingAdditivePassthrough*/), smoothstep((0.3 /*_LightingAdditiveGradientStart*/), (0.5 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation * detailShadow;
            #else
                finalLighting = lerp(poiLight.color * max(poiLight.attenuation, (0.005 /*_LightingAdditivePassthrough*/)), poiLight.color * (0.005 /*_LightingAdditivePassthrough*/), smoothstep((0.3 /*_LightingAdditiveGradientStart*/), (0.5 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5))) * detailShadow;
            #endif
        }
        else //if((1.0 /*_LightingAdditiveType*/) == 2) // Wrapped
        {
            ...
            if ((0.0 /*_LightingRampType*/) == 1) // Math Gradient
            poiLight.rampedLightMap = lerp(float4(1,1,1,1), float3(1, 1, 1), saturate(1 - smoothstep((0.0 /*_LightingGradientStart*/) - .000001, (0.5 /*_LightingGradientEnd*/), 1 - uv)));
            float shadowatten = max(poiLight.additiveShadow, (0.005 /*_LightingAdditivePassthrough*/));
        ...
        switch((1.0 /*_LightingMode*/))
        {
            ...
            case 2: // or wrapped
            {
                ...
                if ((0.005 /*_LightingMinLightBrightness*/) > 0)
                {
                    poiLight.finalLighting = max(0.001, poiLight.finalLighting);
                    float finalluminance = calculateluminance(poiLight.finalLighting);
                    finalLighting = max(poiLight.finalLighting, poiLight.finalLighting / max(0.0001, (finalluminance / (0.005 /*_LightingMinLightBrightness*/))));
            ...
            case 1: // realistic
            {
                ...
                finalLighting = lerp(realisticLighting, dot(realisticLighting, float3(0.299, 0.587, 0.114)), (0.1 /*_LightingMonochromatic*/));
            }
            break;
            case 3: // Skin
            {
                ...
                finalLighting = min(lerp(indirectLighting * float4(1,1,1,1), float4(1,1,1,1), (1.0 /*_LightingIgnoreAmbientColor*/)) + (sss * directLighting), directLighting);
            }
            break;
            case 4:
            {
